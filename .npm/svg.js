...
// Папка с SVG
const inputDir = path.resolve(__dirname, '../resources/icons/collection');
// Куда сохранить спрайт
const outputDir = path.resolve(__dirname, '../resources/icons/sprite');
const outputDirBlade = path.resolve(__dirname, '../resources/views/partials');

const config = {
  mode: {
    symbol: {
      // создаем <symbol> спрайт
...
spriter.compile((error, result) => {
  ...
  fs.writeFileSync(
    path.join(outputDirBlade, 'svg-sprite.blade.php'),
    spriteContent,
  );
