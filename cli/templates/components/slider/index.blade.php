...
@php
    ...
    // Design System: Семантические классы для стрелок
    $btnClasses = 'cursor-pointer p-2 bg-background/80 backdrop-blur rounded-full shadow-sm border border-border text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ring-offset-background';
@endphp
...
<div class="wp-swiper-root {{ $class }}">
    ...
    >
        ...
        @if($navigationIsDefault)
            <div class="wp-swiper-prev absolute left-2 top-1/2 -translate-y-1/2 z-10 {{ $btnClasses }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                </svg>
            </div>
            <div class="wp-swiper-next absolute right-2 top-1/2 -translate-y-1/2 z-10 {{ $btnClasses }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
