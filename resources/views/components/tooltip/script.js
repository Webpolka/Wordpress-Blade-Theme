/**
 * Компонент Tooltip для Alpine.js
 *
 * Всплывающая подсказка с поддержкой:
 * - 4 позиций (top, bottom, left, right)
 * - Задержки появления/искрытия
 * - Стрелки (опционально)
 * - Тем (dark, light) и кастомных цветов
 * - Автоматическое вычисление среднего цвета для градиентов
...
class TooltipComponent {
  ...
  constructor(props) {
    ...
    this.distance = props.distance ?? 0.5;  // ← расстояние в rem (по умолчанию 8)
  ...
  get positionClasses() {
    // Убрали mb-2/mt-2/mr-2/ml-2 — теперь расстояние управляется через distanceStyle
    const positions = {
  ...
  /**
   * Inline style для расстояния от элемента
   */
  get distanceStyle() {
  ...
  get arrowColorClasses() {
    // Если указан color - парсим bg- класс (без градиентов)
    if (this.color) {
      // Проверяем что это НЕ градиент
      ...
      // Для градиентов используем inline style через arrowBgStyle
      return '';
    }

    // Иначе используем theme
    if (this.theme === 'light') {
      return 'bg-white border-t border-l border-gray-200';
    }
  ...
  /**
   * Inline style для стрелки (используется для градиентов)
   */
  get arrowBgStyle() {
    ...
    // Если только from — берём его
    if (fromMatch) {
      const fromHex = TooltipComponent.COLOR_MAP[`${fromMatch[1]}-${fromMatch[2]}`];
      if (fromHex) return `background: ${fromHex}`;
    }
    
    // Если только to — берём его
    if (toMatch) {
      const toHex = TooltipComponent.COLOR_MAP[`${toMatch[1]}-${toMatch[2]}`];
  ...
  /**
   * Маппинг популярных Tailwind цветов в HEX
   */
  static COLOR_MAP = {
    // Slate
   ...
   * @param {string} color1 - Первый HEX цвет (#RRGGBB)
   * @param {string} color2 - Второй HEX цвет (#RRGGBB)
   * @param {number} ratio - Соотношение (0 = только color1, 1 = только color2, 0.5 = середина)
...
export function registerTooltip() {
  if (typeof window.Alpine === 'undefined') {
    console.warn('⚠️ Alpine не загружен, компонент Tooltip не зарегистрирован');
    return;
  }
  window.Alpine.data('tooltip', (props) => new TooltipComponent(props));
  console.log('✅ Tooltip компонент зарегистрирован');
}

if (typeof window.Alpine !== 'undefined') {
  registerTooltip();
} else {
  document.addEventListener('alpine:init', () => registerTooltip());
}
